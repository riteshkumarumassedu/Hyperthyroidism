...
def affine_forward(x, w, b):
    ...
    # convert to vector
    new_x= np.reshape(x,[num_images,-1])
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    running_var = momentum * running_var + (1 - momentum) * mb_variance
    ...
    if mode == 'train':
        ...
        # compute the mean and variance for minibatch
        mb_mean = np.mean(x, axis=0)
        mb_variance = np.var(x, axis=0)

        # normalize the data
        normalized_data = (x - mb_mean) / np.sqrt(mb_variance + eps)
        
        # store output in 'out' var as mentioned
        ...
        # update the bn_params with updated values
        ...
        cache = {
            ...
            'gamma': gamma,
            'variance_inv': 1.0/np.sqrt(mb_variance + eps),
            'sqrtvar': np.sqrt(mb_variance + eps),
        }
...
def batchnorm_backward(dout, cache):
    ...
    variance_inv = cache.get('variance_inv')
    ...
    # compute dx1
    dxhat = dout*gamma
    dxmu1 = dxhat*variance_inv
    
    # compute grad of variance inverse and var_sqrt
    dvariance_inv = np.sum(dxhat*x_mean, axis=0)
    dsqrtvar = dvariance_inv * (-1/sqrtvar**2)
...
def batchnorm_backward_alt(dout, cache):
    ...
    # compute dbeta and dgamma from out backprop
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * normalized_data, axis=0)

    # Alternative faster formula way of calculating dx. ref: http://cthorey.github.io./backpropagation/
    dx =(1/N)*gamma*1/sqrtvar*((N * dout)-np.sum(dout,axis=0)-(x_mean)*np.square(variance_inv)*np.sum(dout*(x_mean), axis=0))
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    #take transpose of normalized data
...
def layernorm_backward(dout, cache):
    ...
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(normalized_data*dout, axis=0)

    dnormalized_data = dout * gamma

    # Transpose dnormalized_data and normalized_data back
    dnormalized_data = dnormalized_data.T
    normalized_data = normalized_data.T

    N, D = normalized_data.shape

    # Copy from batch normalization backward code
    dx = 1.0/N * variance_inv * (N*dnormalized_data - np.sum(dnormalized_data, axis=0) - normalized_data*np.sum(dnormalized_data*normalized_data, axis=0))
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        # creating the mask as per notes
        # since p is probability to "keep" then the probablity to drop would be 
        ...
        #######################################################################
        # in test phase we are not suppoed to apply any dropout
        out = x
        #######################################################################
        #                            END OF YOUR CODE                         #
        #######################################################################
...
def conv_backward_naive(dout, cache):
    ...
    # fetch the conv params
    stride = conv_param['stride']
    pad = conv_param['pad']
    
    # fetch the attributes from cache
    x, w, b, conv_param = cache
    N, C, H, W = x.shape
    F, C_two, HH, WW = w.shape
    F_two, C_two, H_out, W_out = dout.shape
...
def max_pool_forward_naive(x, pool_param):
    ...
    # fetching the shapes and pool_params
    N, C, H, W = x.shape
    pool_height = pool_param['pool_height']
    pool_width = pool_param['pool_width']
    stride = pool_param['stride']
    
    # applying as mentioned above
...
def max_pool_backward_naive(dout, cache):
    ...
    ###########################################################################
    # get x and pool params from the cache
    x, pool_param = cache
    N, C, H, W = x.shape
    pool_height = pool_param['pool_height']
    pool_width = pool_param['pool_width']
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    # Reshaping x to N*H*W * C for batch normalization
    x_new = np.reshape(np.transpose(x, (0, 2, 3, 1)), (-1, C))
    out, cache = batchnorm_forward(x_new, gamma, beta, bn_param)
    # Reshaping out to (N, C, H, W)
